...
def masked_accuracy(y_true, y_pred): #custom accuracy function
    
    mask = tf.math.not_equal(y_true, 0)  #boolean mask where elements of y_true that are != 0 are marked as True and those = 0 are marked as False --> filters out padding elements labeled "0"
    y_true_masked = tf.boolean_mask(y_true, mask) #apply mask to ground truth --> to extract only elements which are not labeled '0'
    y_pred_masked = tf.boolean_mask(y_pred, mask) #apply mask to predicted values --> to extract only elements which are not labeled '0'
    
    return tf.keras.metrics.sparse_categorical_accuracy(y_true_masked, y_pred_masked) #calculates accuracy using keras's 'sparse_categorical_accuracy' --> calculates how often predictions match integer labels

ner_model.compile(optimizer="adam", loss=loss, metrics=[masked_accuracy])
ner_model.fit(train_dataset, epochs=10, callbacks=[history])

# Access the loss and accuracy values
loss_values = history.history['loss']
accuracy_values = history.history["masked_accuracy"]
